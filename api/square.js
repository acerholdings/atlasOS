  ...
  try {
    ...
    if (action === 'status') {
      ...
      const connRes = await fetch(
        `${SUPABASE_URL}/rest/v1/square_connections?business_id=eq.${b.businessId}&select=location_id,token_expires_at,updated_at`,
        { headers: { 'Authorization': `Bearer ${SERVICE_KEY}`, 'apikey': SERVICE_KEY } }
      );
      const [conn] = await connRes.json();
      if (!conn) return res.status(200).json({ connected: false });
      return res.status(200).json({ connected: true, location_id: conn.location_id, token_expires_at: conn.token_expires_at || null, updated_at: conn.updated_at || null });
    }
